		...
		svg.selectAll("[data-legend]").each(function() {
			...
			items[self.attr("data-legend")] = {
				...
				color: self.attr("data-legend-color") != undefined ?
					self.attr("data-legend-color") : self.style("fill") != 'none' ? self.style("fill") : self.style("stroke"),
				icon: "line"
			}
		})
...
function createChart(container, receivedData, minTemp, maxTemp) {
	...
	for (iii = 0; iii < receivedData.length; iii++) {
		data.push({
			date: receivedData[iii].CREATION_TS,
			close: receivedData[iii].TEMP
		});
	}
	...
	var bottomY = minTemp.toString();
	var topY = maxTemp.toString();
	...
	svg.append("path")
		.attr("d", valueline([{
			...
			close: bottomY
		}])).attr('class', 'bottom-line').attr("data-legend", "Min");
	...
	svg.append("path").attr("class", "line")
		.attr("d", valueline([{
			...
			date: data[data.length - 1].date,
			close: topY
		}])).attr('class', 'top-line').attr("data-legend", "Max");
